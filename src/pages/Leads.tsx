...
const statusVariant = (status: LeadStatus) => {
  if (status === "CONVERTIDO") return "default";
...
export default function Leads() {
    ...
    queryFn: async () => {
      ...
      const { data, error } = await sb
        ...
        .select(`
          ...
          payment_type,
          order_id,
          excursions(name,destination,departure_at),
          affiliates(name)
        `)
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: rows.length,
      capturados: rows.filter((r) => r.status === "CAPTURADO").length,
      pixGerado: rows.filter((r) => r.status === "PIX_GERADO").length,
      convertidos: rows.filter((r) => r.status === "CONVERTIDO").length,
    };
  }, [leads]);
  ...
  return (
    <MainLayout>
      ...
      <div className="page-header">
        <h1 className="page-title">Leads de Excursoes</h1>
        <p className="page-subtitle">Acompanhamento do funil publico de reserva e pagamento</p>
      </div>
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4 mb-6">
        ...
        <Card><CardContent className="pt-4 pb-4"><p className="text-xs text-muted-foreground">Convertidos</p><p className="text-2xl font-bold text-emerald-500">{stats.convertidos}</p></CardContent></Card>
      </div>
      ...
      <Card className="mb-4">
        <CardContent className="pt-4 pb-4">
          <div className="grid gap-3 md:grid-cols-[1fr_240px]">
            <Input
              placeholder="Buscar por nome, CPF, telefone, excursao ou afiliado..."
              value={search}
                ...
                {filtered.map((l) => (
                  <TableRow key={l.id}>
                    ...
                    <TableCell className="text-sm">
                      {l.payment_type || "-"}
                      {l.order_id ? <div className="text-xs text-muted-foreground">Pedido vinculado</div> : null}
                    </TableCell>
                    <TableCell className="text-sm">
                      {l.source}
                      {l.affiliates?.name ? <div className="text-xs text-muted-foreground">Afiliado: {l.affiliates.name}</div> : null}
                    </TableCell>
